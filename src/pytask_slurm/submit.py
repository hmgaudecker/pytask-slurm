...
def _warn_on_conflicting_extra(
    tokens: list[str],
    generated_flags: frozenset[str] | None = None,
) -> None:
    """Warn if --slurm-extra contains flags that are auto-generated by pytask-slurm."""
    flags_to_check = generated_flags if generated_flags is not None else _GENERATED_SBATCH_FLAGS
    for token in tokens:
        short = _resolve_short_flag(token)
        flag = _SHORT_TO_LONG[short] if short is not None else token.split("=")[0]
        if flag in flags_to_check:
            logger.warning(
...
def _build_sbatch_cmd(
    ...
    *paths* is ``(log_path, payload_path, result_path, script_path)``.
    ...
    if extra:
        ...
        generated = (
            _GENERATED_SBATCH_FLAGS | frozenset(extra_mark_flags)
            if extra_mark_flags
            else None
        )
        _warn_on_conflicting_extra(extra_tokens, generated_flags=generated)
